...
To add a station you need three things:
- `value` — the station ID (find it on websites like EVE Ref or Dotlan)
- `data-region` — the region ID the station is in
- `data-short` — a short display name used in flags and tooltips

Copy one of the existing lines, change those three values, and paste it in both dropdowns.
